...
class DashBoardQueries {
    ...
    async productsTopFivePopular(top) {
        try {
            const conn = await database_1.default.connect();
            const sql = 'SELECT name, category, price, quantity FROM products INNER JOIN order_products ON products.id = order_products.product_id ORDER BY quantity DESC LIMIT $1;';
    ...
    async currentOrderByUser(userId) {
        try {
            const conn = await database_1.default.connect();
            const sql = "SELECT product_id, quantity, status FROM orders INNER JOIN order_products ON orders.id=order_products.order_id WHERE orders.user_id=($1) AND orders.status='active';";
